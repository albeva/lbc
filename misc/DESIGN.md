...
- Interop with C libraries via simple declarations (like header files)
...
- `src/Diag/Diagnostics.td` — diagnostic message catalog (see Diagnostics below).
  Generates the `DiagCategory` enum, `DiagMessage` struct, and type-safe factory
  functions in the `Diagnostics` namespace (`lbc-diag-def`).
...
`DiagResult<T>` = `std::expected<T, DiagIndex>` — trivially copyable (4 bytes),
propagated by the `TRY` / `TRY_ASSIGN` / `TRY_DECL` / `MUST` macros unchanged. The
engine owns all diagnostic details; the error return path carries only the index.

### Key types

- **`DiagMessage`** — generated struct carrying severity (`llvm::SourceMgr::DiagKind`),
  category (`DiagCategory`), diagnostic code, and a pre-formatted message string.
- **`DiagIndex`** — opaque handle (4 bytes) into `DiagEngine` storage. Default-constructed
  instances are invalid (sentinel). Only `DiagEngine` can create valid indices.
- **`DiagResult<T>`** — alias for `std::expected<T, DiagIndex>`.
- **`DiagError`** — alias for `std::unexpected<DiagIndex>`, returned directly from
  helper methods.
- **`DiagEngine`** — accumulates diagnostics and pairs each `DiagMessage` with an
  `llvm::SMDiagnostic` for rendering.
- **`LogProvider`** — convenience mixin using C++23 deducing this. Any class that
  satisfies the `ContextAware` concept (exposes `getContext()`) inherits a `diag()`
  helper that logs a message and returns `DiagError` in one step.

### Flow

1. Pass calls `diag(Diagnostics::unexpectedToken(token), loc)` → gets `DiagError`.
2. Returns `DiagError` directly, or propagates via `TRY` / `TRY_ASSIGN` / `TRY_DECL`.
3. Accumulated diagnostics are rendered after the pass completes via `DiagEngine::print()`.
...
a stable diagnostic code, and a format string with typed placeholders:

- `{name}` — parameter constrained by the `Loggable` concept (`std::formattable<T, char>`)
- `{name:type}` — parameter with an explicit C++ type (e.g. `{count:int}`)

The `lbc-diag-def` generator (`DiagGen`) parses these placeholders and emits type-safe
factory functions in the `Diagnostics` namespace. Messages without placeholders produce
plain string literals; messages with placeholders use `std::format`.
