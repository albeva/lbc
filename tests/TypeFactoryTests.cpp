...
TEST(TypeFactoryTests, ConstQualifier) {
    Context context;
    auto& tf = context.getTypeFactory();

    const auto* constInt = tf.getConst(tf.getInteger());
    EXPECT_TRUE(constInt->isConst());
    EXPECT_FALSE(constInt->isVolatile());
    EXPECT_EQ(constInt->getBaseType(), tf.getInteger());
}

TEST(TypeFactoryTests, VolatileQualifier) {
    Context context;
    auto& tf = context.getTypeFactory();

    const auto* volInt = tf.getVolatile(tf.getInteger());
    EXPECT_FALSE(volInt->isConst());
    EXPECT_TRUE(volInt->isVolatile());
    EXPECT_EQ(volInt->getBaseType(), tf.getInteger());
}

TEST(TypeFactoryTests, ConstVolatileQualifier) {
    Context context;
    auto& tf = context.getTypeFactory();

    const auto* cvInt = tf.getConst(tf.getVolatile(tf.getInteger()));
    EXPECT_TRUE(cvInt->isConst());
    EXPECT_TRUE(cvInt->isVolatile());
    EXPECT_EQ(cvInt->getBaseType(), tf.getInteger());
}

TEST(TypeFactoryTests, ConstVolatileOrderIndependent) {
    Context context;
    auto& tf = context.getTypeFactory();

    const auto* cv = tf.getConst(tf.getVolatile(tf.getInteger()));
    const auto* vc = tf.getVolatile(tf.getConst(tf.getInteger()));
    EXPECT_EQ(cv, vc);
}

TEST(TypeFactoryTests, QualifiedStability) {
    Context context;
    auto& tf = context.getTypeFactory();

    EXPECT_EQ(tf.getConst(tf.getInteger()), tf.getConst(tf.getInteger()));
    EXPECT_EQ(tf.getVolatile(tf.getBool()), tf.getVolatile(tf.getBool()));
}

TEST(TypeFactoryTests, DistinctQualifiers) {
    Context context;
    auto& tf = context.getTypeFactory();

    EXPECT_NE(tf.getConst(tf.getInteger()), tf.getVolatile(tf.getInteger()));
}

TEST(TypeFactoryTests, RedundantConstCollapsed) {
    Context context;
    auto& tf = context.getTypeFactory();

    const auto* constInt = tf.getConst(tf.getInteger());
    const auto* constConstInt = tf.getConst(constInt);
    EXPECT_EQ(constInt, constConstInt);
}
