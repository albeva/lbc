 ...
 */
void AstVisitorGen::visitorClass(const lib::TreeNode* ast) {
    ...
    doc([&] {
        ...
        block("class " + sampleName + " final : " + visitorName + "<>", true, [&] {
            ...
            newline();
            ast->visit(lib::TreeNode::Kind::Leaf, [&](const lib::TreeNode* node) {
...
void AstVisitorGen::visit(const lib::TreeNode* klass) {
 ...
 */
void AstVisitorGen::caseAccept(const lib::TreeNode* klass) {
...
void AstVisitorGen::visitFunction() {
    ...
    const auto childdoc = [&](const lib::TreeNode* child) {
        if (child->isLeaf()) {
    ...
    doc([&] {
        ...
        block("const auto visitor = Visitor", true, [&] {
            ...
            root->visit(lib::TreeNode::Kind::Group, [&](const lib::TreeNode* group) {
 ...
 */
void AstVisitorGen::caseForward(const lib::TreeNode* klass) {
    line("case AstKind::" + klass->getEnumName(), ":");
